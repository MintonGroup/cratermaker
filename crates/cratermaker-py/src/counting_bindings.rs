...
pub fn fit_one_ellipse<'py>(
    ...
    let (x0, y0, ap, bp, orientation, wrms) =
        cratermaker_components::counting::fit_one_ellipse(x_v, y_v, weights_v)
...
pub fn fit_one_ellipse_fixed_center<'py>(
    ...
    let y_v = y.as_array();
    let weights_v = weights.as_array();
    let (ap, bp, orientation, wrms) =
        cratermaker_components::counting::fit_one_ellipse_fixed_center(x_v, y_v, weights_v, x0, y0)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn score_rim<'py>(
    ...
    let x0y0 = transformer.call_method1(
        "transform",
        (crater.measured_location.0, crater.measured_location.1),
    )?;
    ...
    let result = cratermaker_components::counting::score_rim(
        &region_v, x0, y0, &crater, quantile, gradmult, curvmult, heightmult,
    )
    ...
    region.call_method1(
        ...
        (
            "rimscore",
            PyArray1::from_owned_array(py, result.clone()),
            "Rim Score",
            "dimensionless",
            true,
            true,
...
pub fn fit_rim<'py>(
    ...
    loop {
        ...
        region = surface.call_method1(
            "extract_region",
            (
                crater_fit.measured_location,
                _FITTING_RADIUS_RATIO * crater_fit.measured_radius,
            ),
        )?;
        if region.is_none() {
            return Err(PyValueError::new_err(
                "Failed to extract region for rim fitting.",
            ));
        }
        // Ensure face projections are set
        region.call_method0("set_face_proj")?;
        ...
        let x0y0 = transformer.call_method1(
            "transform",
            (
                crater_fit.measured_location.0,
                crater_fit.measured_location.1,
            ),
        )?;
        let (x0, y0): (f64, f64) = x0y0.extract()?;
        ...
        (x0_fit, y0_fit, a_fit, b_fit, o_fit, rimscore) =
            cratermaker_components::counting::fit_one_rim(
                &region_v,
                x0,
                y0,
                &crater_fit,
    ...
    region.call_method1(
        ...
        (
            "rimscore",
            PyArray1::from_owned_array(py, rimscore.clone()),
            "Rim Score",
            "dimensionless",
            true,
            true,
