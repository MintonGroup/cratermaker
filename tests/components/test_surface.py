...
class TestSurface(unittest.TestCase):
    ...
    def test_calculate_haversine_distance(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as simdir:
            ...
            reference_location = (0, 0)  # Equator, prime meridian
            ...
            calculated_distance = surface.compute_distances(reference_location, locations)[0]
    ...
    def test_compute_location_from_distance_bearing(self):
        # Example coordinates (lat/lon in radians)
        reference_locations = [(0, 0), (180, 0), (270, 0), (0, 90), (0, -90), (45, 45)]
        reference_locations = [(0, 0), (180, 0), (270, 0), (0, 89.99), (0, -89.99), (45, 45)]
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as simdir:
            ...
            for reference_location in reference_locations:
                for distance in distances:
                    ...
                    computed_bearings = surface.compute_bearings(
                        reference_location=reference_location, locations=computed_locations
                    )
                    ...
                    np.testing.assert_array_almost_equal(
                        ...
                        dist_arr,
                        decimal=5,
                        err_msg=f"Center location: {reference_location}, Distance: {distance}, Distance differences: {computed_distances - dist_arr}",
                    )
                    np.testing.assert_array_almost_equal(
                        computed_bearings,
                        bearings,
                        decimal=5,
                        err_msg=f"Center location: {reference_location}, Distance: {distance}, Bearing differences: {computed_bearings - bearings}",
                    )
