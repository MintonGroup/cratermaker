...
class TestCrater(unittest.TestCase):
    ...
    def test_crater_initialization_with_diameter(self):
        final_diameter = 1000
        crater = Crater.make(final_diameter=final_diameter)
    ...
    def test_crater_initialization_with_radius(self):
        final_radius = 500
        crater = Crater.make(final_radius=final_radius)
    ...
    def test_crater_initialization_with_transient_diameter(self):
        ...
        crater = Crater.make(transient_diameter=transient_diameter)
    ...
    def test_crater_initialization_with_transient_radius(self):
        ...
        crater = Crater.make(transient_radius=transient_radius)
        self.assertEqual(crater.transient_radius, transient_radius)

    def test_invalid_negative_values(self):
        with self.assertRaises(ValueError):
            Crater.make(final_diameter=-1000)

    def test_invalid_combinations(self):
        with self.assertRaises(ValueError):
            Crater.make(final_diameter=1000, final_radius=500)

    def test_non_default_target_and_rng(self):
        rng = default_rng()
        target = Target("Mars")
        crater = Crater.make(final_diameter=1000,target=target,rng=rng)
        self.assertEqual(crater.final_diameter, 1000)

    def test_invalid_target_or_rng_type(self):
        with self.assertRaises(ValueError):
            Crater.make(final_diameter=1000, target="invalid_target")

    def test_projectile_initialization_with_diameter(self):
        diameter = 1000
        crater = Crater.make(projectile_diameter=diameter, projectile_velocity=20e3)
    ...
    def test_projectile_initialization_with_radius(self):
        radius = 500
        crater = Crater.make(projectile_radius=radius, projectile_velocity=20e3)
        self.assertEqual(crater.projectile_radius, radius)
        self.assertEqual(crater.projectile_diameter, radius * 2)
        
    def test_invalid_negative_values(self):
        with self.assertRaises(ValueError):
            Crater.make(projectile_diameter=-1000, projectile_velocity=20e3)

    def test_invalid_combinations(self):
        with self.assertRaises(ValueError):
            Crater.make(projectile_diameter=1000, projectile_radius=500, projectile_velocity=20e3)
        with self.assertRaises(ValueError):
            Crater.make(projectile_diameter=1000, final_diameter=10000)
    
    def test_non_default_target_and_rng(self):
        rng = default_rng()
        target = Target("Mars") 
        crater = Crater.make(projectile_diameter=1000,target=target,rng=rng,projectile_mean_velocity=10e3)
        self.assertEqual(crater.projectile_diameter, 1000)

    def test_invalid_target_or_rng_type(self):
        with self.assertRaises(ValueError):
            Crater.make(projectile_diameter=1000, target="invalid_target", projectile_mean_velocity=10e3)
