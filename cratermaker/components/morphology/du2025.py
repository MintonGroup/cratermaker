    ...
    **kwargs : Any
    ...
    def _get_1D_power_spectral_density(self, feature, psd_coef, num_psd_component_effec=100) -> NDArray:
        ...
        if self.crater.final_diameter < psd_coef["1D"][feature]["slope_12"]["D_tie"]:
            slope_12 = (
                psd_coef["1D"][feature]["slope_12"]["k1"] * self.crater.final_diameter + psd_coef["1D"][feature]["slope_12"]["b1"]
            )
            slope_12_sigma = psd_coef["1D"][feature]["slope_12"]["sigma_1"]
        else:
            slope_12 = (
                psd_coef["1D"][feature]["slope_12"]["k2"] * self.crater.final_diameter + psd_coef["1D"][feature]["slope_12"]["b2"]
            )
    ...
    def _get_2D_power_spectral_density(
        ...
        if self.crater.final_diameter < psd_coef["2D"][feature]["nu_fall"]["D_tie"]:
            nu_fall = (
                psd_coef["2D"][feature]["nu_fall"]["k1"] * self.crater.final_diameter + psd_coef["2D"][feature]["nu_fall"]["b1"]
            )
            nu_fall_sigma = psd_coef["2D"][feature]["nu_fall"]["sigma_1"]
        else:
            nu_fall = (
                psd_coef["2D"][feature]["nu_fall"]["k2"] * self.crater.final_diameter + psd_coef["2D"][feature]["nu_fall"]["b2"]
            )
            nu_fall_sigma = psd_coef["2D"][feature]["nu_fall"]["sigma_2"]
        if self.crater.final_diameter < psd_coef["2D"][feature]["psd_sigma"]["D_tie"]:
            psd_sigma = (
                psd_coef["2D"][feature]["psd_sigma"]["k1"] * self.crater.final_diameter + psd_coef["2D"][feature]["psd_sigma"]["b1"]
            )
            psd_sigma_sigma = psd_coef["2D"][feature]["psd_sigma"]["sigma_1"]
        else:
            psd_sigma = (
                psd_coef["2D"][feature]["psd_sigma"]["k2"] * self.crater.final_diameter + psd_coef["2D"][feature]["psd_sigma"]["b2"]
            )
        ...
        if feature == "wall":
            if self.crater.final_diameter < psd_coef["2D"][feature]["E_circ"]["D_tie"]:
                E_circ = (
                    psd_coef["2D"][feature]["E_circ"]["k1"] * self.crater.final_diameter + psd_coef["2D"][feature]["E_circ"]["b1"]
                )
                E_circ_sigma = psd_coef["2D"][feature]["E_circ"]["sigma_1"]
            else:
                E_circ = (
                    psd_coef["2D"][feature]["E_circ"]["k2"] * self.crater.final_diameter + psd_coef["2D"][feature]["E_circ"]["b2"]
                )
