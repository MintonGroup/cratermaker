...
if __name__ == "__main__":
    ...
    def plot_npf_csfd():
        ...
        ax = {"Moon": fig.add_subplot(121), "Mars": fig.add_subplot(122)}
        ...
        for key in ax:
            ...
            ax[key].yaxis.set_minor_locator(
            ...
            ax[key].xaxis.set_minor_locator(
                ticker.LogLocator(base=10.0, subs=np.arange(2, 10), numticks=100)
            )
            ...
            for t in tvals:
                ...
                ax[key].plot(
                    Dvals[lo], Nvals[lo], "-.", color="orange", linewidth=2.0, zorder=50
                )
                ax[key].plot(
                    Dvals[hi], Nvals[hi], "-.", color="orange", linewidth=2.0, zorder=50
                )
                labeli = int(0.25 * nD)
                ax[key].text(
                    Dvals[labeli],
    ...
    def plot_npf_N1_vs_T():
        ...
        ax.plot(
            tvals, N1_moon, "-", color="dimgrey", linewidth=2.0, zorder=50, label="Moon"
        )
        ax.plot(
            tvals, N1_mars, "-", color="orange", linewidth=2.0, zorder=50, label="Mars"
        )
        ax.legend()
        plt.tight_layout()
        plt.show()
    ...
    def plot_npf_fit():
        ...
        projectile_production = NeukumProduction(
            version="Projectile", impact_velocity_model="Moon_MBA"
        )
        ...
        # Output the results
        print(
            "Crater Production Fit Parameters N1_coef =",
            N1_coef_crater,
            ", slope =",
            slope_crater,
        )
        print(
            "Projectile Production Fit Parameters N1_coef =",
            N1_coef_projectile,
            ", slope =",
            slope_projectile,
        )
    ...
    def plot_sampled_csfd():
        ...
        fig = plt.figure(1, figsize=(8, 7))
        ax = {"Power Law": fig.add_subplot(121), "NPF (Moon)": fig.add_subplot(122)}
        ...
        for key in ax:
            ...
            for i in range(Nevaluations):
                Dsampled, _ = production[key].sample(
