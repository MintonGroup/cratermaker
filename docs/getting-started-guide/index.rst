...
#####################################
Visualizing the surface with ParaView
#####################################
...
To visualize the surface mesh, first make sure you have `ParaView <https://www.paraview.org/>` installed. The two files will be 
``staticFieldsOnCells.vtp``, which are the variables associated with mesh cells (faces) and ```staticFieldsOnVertices.vtp``, which are the
variables associated with mesh vertices (nodes).
